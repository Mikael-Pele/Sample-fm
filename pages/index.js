...
// A creator's "custom_domain" (set in the dashboard) only means something if
// visiting that domain actually lands somewhere useful. This is that other
// half: if the request's Host header isn't droppa.fm's own domain, check
// whether it's a creator's custom domain and, if so, send the root path
// straight to their most recent SmartLink instead of showing the generic
// sign-up homepage. This still requires the creator to have (a) pointed
// their domain's DNS at this deployment and (b) added it in Vercel's
// project settings — neither of those can happen from code, since DNS is
// controlled by whoever owns the domain, not by us.
function getAppHost() {
...
export async function getServerSideProps({ req, query }) {
  ...
  if (!isKnownAppHost) {
    ...
    if (owner) {
      ...
      // Domain is correctly set up, but this creator has no SmartLinks yet
      // — fall through to the normal homepage rather than a dead end.
    }
  }
  ...
  return (
    <div className="relative min-h-screen bg-base-bg text-white overflow-hidden">
      ...
      <Head>
        ...
        <meta
          name="description"
          content="Droppa.fm is the elite SmartLink and pre-save platform built for independent music creators and labels, with Paystack billing built in."
        />
      </Head>
      ...
      <footer className="text-center text-base-muted text-xs pb-10 space-y-3">
        <p>Droppa.fm — SmartLinks &amp; pre-saves engineered for independent music creators.</p>
        <SiteFooter />
      </footer>
    </div>
  );
