...
// env var silently produces a broken relative callback_url ("/billing/
// callback" instead of "https://yoursite.com/billing/callback"), which
// Paystack rejects outright — that's what "Could not start checkout" meant.
  ...
  try {
    ...
    if (!paystackRes.ok || !paystackData.status || !paystackData.data) {
      ...
      return res.status(502).json({
        error: paystackData && paystackData.message ? paystackData.message : "Could not start checkout. Please try again.",
      });
    }
