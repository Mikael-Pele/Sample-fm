...
// One Paystack Payment Page per price point (2 regions x monthly/yearly) —
// see .env.example for how to set each of these up, and lib/plans.js for
// the pricing they must match.
const PLAN_URLS = {
  africa_monthly: process.env.NEXT_PUBLIC_PAYSTACK_URL_AFRICA_MONTHLY || "#",
  africa_yearly: process.env.NEXT_PUBLIC_PAYSTACK_URL_AFRICA_YEARLY || "#",
  global_monthly: process.env.NEXT_PUBLIC_PAYSTACK_URL_GLOBAL_MONTHLY || "#",
  global_yearly: process.env.NEXT_PUBLIC_PAYSTACK_URL_GLOBAL_YEARLY || "#",
};
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-5">
            <div className="min-w-0">
              ...
              <p className="text-sm text-base-muted max-w-md">
                {isPro
                  ? `Unlimited SmartLinks, retargeting pixels, custom domains, fan email exports, and zero Sample.fm branding are unlocked.${
                  ...
                  : `Up to ${FREE_TIER_LINK_LIMIT} SmartLinks with the Sample.fm badge. Upgrade to unlock unlimited links, retargeting pixels, custom domains, and your fan email database.`}
              </p>
            </div>
            {!IS_PRODUCTION && (
              <button
          ...
          {!isPro && (
            <div className="space-y-4">
              ...
              <div className="max-w-sm bg-base-bg border-2 border-brand rounded-xl p-5 flex flex-col">
                ...
                <a
                  href={PLAN_URLS[`${pricingRegion}_${billingInterval}`]}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ "--glow-color": "rgba(255, 77, 0, 0.5)" }}
                  className="shimmer-gold glow-on-hover text-center text-base-bg font-bold rounded-lg py-2.5 text-sm"
                >
                  Upgrade to Premium
                </a>
                <button
                  type="button"
                  onClick={() => setPricingRegion((r) => (r === "africa" ? "global" : "africa"))}
                  className="mt-3 text-[11px] text-base-muted hover:text-white transition text-center"
                >
                  {pricingRegion === "africa"
                    ? "Not in Africa? See global pricing"
                    : "Based in Africa? See local pricing"}
                </button>
              </div>
            </div>
          )}
