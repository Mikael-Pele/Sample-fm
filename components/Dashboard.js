        ...
        <section id="billing" className="glass-card rounded-xl2 p-5 sm:p-6 scroll-mt-24">
          ...
          {isPro && user?.custom_domain ? (
            <div className="mt-4 pt-4 border-t border-base-border text-xs text-base-muted space-y-1.5">
              ...
              <p>
                1. At your domain registrar, point <span className="text-white">{user.custom_domain}</span> at
                this app — add a CNAME record with value{" "}
                <code className="text-brand-light">cname.vercel-dns.com</code> (or, for a root/apex domain, an
                A record to <code className="text-brand-light">76.76.21.21</code>).
              </p>
              <p>
                2. Add <span className="text-white">{user.custom_domain}</span> under Settings → Domains in the
                Vercel project this site is deployed on. Vercel will confirm once DNS is detected — that step
                can&rsquo;t be done from here, since only the site&rsquo;s owner has access to that project.
              </p>
              <p>
                Once both are done, visiting {user.custom_domain} will automatically land on your most recent
                SmartLink.
              </p>
            </div>
          ) : null}
          ...
          {isPro && (
            <div className="mt-4 pt-4 border-t border-base-border flex flex-wrap items-center gap-x-5 gap-y-2 text-xs">
