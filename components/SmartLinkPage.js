...
// Snippet playback length. We deliberately cut the clip short even though
// the sourced preview (Apple/Deezer) is a longer ~30s file — a quick 5s
// taste is enough to spark interest without letting the fan "consume" the
// track for free on our landing page.
const SNIPPET_DURATION_MS = 5000;
...
export default function SmartLinkPage({ smartlink, isAfricanFan, fanCountry, ownerIsPro }) {
  ...
  // Stops playback and clears the 5-second cutoff timer — used both when
  // the fan pauses manually and when the snippet naturally times out.
  ...
  function handlePreviewToggle() {
    if (!smartlink.preview_url || !previewAudioRef.current) return;
    ...
    previewAudioRef.current
      ...
      .then(() => {
        ...
        trackClick("preview_play");
        previewTimerRef.current = setTimeout(stopPreview, SNIPPET_DURATION_MS);
      })
      .catch(() => {
        // Autoplay-policy or decode failure — fail silently, button just
        // stays in its "play" state so the fan can try again.
      });
  }

  // Stop and release the snippet if the fan navigates away mid-play.
  ...
  const pageTitle = `${smartlink.track_title} — ${smartlink.artist_name} | Droppa.fm`;
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-sm glass-card rounded-xl2 shadow-glass p-5 sm:p-6">
        <div className="relative w-full aspect-square rounded-xl overflow-hidden mb-5 bg-base-card">
          ...
          {smartlink.preview_url && (
            <>
              {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
              <audio
                ref={previewAudioRef}
                src={smartlink.preview_url}
                preload="none"
                onEnded={stopPreview}
              />
              <button
                ...
                aria-label={previewPlaying ? "Pause preview" : "Play 5-second preview"}
                className="absolute top-2.5 right-2.5 w-10 h-10 rounded-full bg-black/50 backdrop-blur flex items-center justify-center text-white hover:bg-black/70 transition"
              >
                {previewPlaying ? (
                  <PauseSnippetIcon size={18} />
                ) : (
                  <PlaySnippetIcon size={18} className="translate-x-[1px]" />
                )}
              </button>
            </>
          )}
        ...
        <div className="mt-3 text-center">
          <ReportProblemTrigger className="text-[11px] text-base-muted/70 hover:text-base-muted transition" />
        </div>
      </div>
