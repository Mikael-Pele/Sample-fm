...
export async function sendSupportReportEmail({ email, message, page_url }) {
  ...
  try {
    await transporter.sendMail({
      from: `"Droppa.fm" <${process.env.GMAIL_USER}>`,
      to,
      replyTo: email || undefined,
      subject: `Droppa.fm — new problem report${email ? ` from ${email}` : ""}`,
      text: [
...
export async function sendPasswordResetEmail({ email, resetUrl }) {
  ...
  try {
    await transporter.sendMail({
      from: `"Droppa.fm" <${process.env.GMAIL_USER}>`,
      to: email,
      subject: "Reset your Droppa.fm password",
      text: [
        "Someone (hopefully you) requested a password reset for your Droppa.fm account.",
        "",
...
export async function sendVerificationEmail({ email, verifyUrl }) {
  ...
  try {
    await transporter.sendMail({
      from: `"Droppa.fm" <${process.env.GMAIL_USER}>`,
      to: email,
      subject: "Confirm your Droppa.fm email",
      text: [
        "Welcome to Droppa.fm — confirm this is your email address:",
        "",
        verifyUrl,
        "",
